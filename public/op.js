...
window.onload = function () {
  ...
  db.collection("OrdenProduccion").orderBy("OrdenP", "desc").limit(300).onSnapshot((querySnapshot) => {
    tabl.innerHTML = "";
    querySnapshot.forEach(function (doc) {
      // doc.data() is never undefined for query doc snapshots
      console.log(doc.id, " => ", doc.data());
...
function RegistrarOP() {
  var query = document.getElementById('orden').value;
  existe = '';
  db.collection("OrdenProduccion").where("OrdenP", "==", query)
    .get()
    .then((querySnapshot) => {
      ...
      if (existe != '') {
        ...
        alert("Ya existe un documento con este ID")
      }else{
