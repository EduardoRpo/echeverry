...
function ventanaSecundaria(URL) {
  window.open(URL, "ventana1", "width=420,height=500,scrollbars=NO")
...
function imprimirOP() {
  document.getElementById("tablaGuia").style.display = "none";
  document.getElementById("tablaActivos").style.display = "none";
  document.getElementById("maquinasTabla").style.display = "none";

  document.getElementById("espacioComen").style.display = "none";


  document.getElementById("tablaOpes").style.width = "600px";
...
//-------------------------------------------------guardar comentarios de ref---------------------------

function guardarComen() {

  if (document.getElementById("consultar").value == "") {
    Swal.fire(
      'Error',
      'Ingrese el número de OP',
      'info'
    )
  }
/*  if(document.getElementById("txtTareaHistorial").value!=""){
    Swal.fire(
      'Error',
      ' Presione el Boton ACTUALIZAR si ya ha hecho un registro',
      'info'
    )
  } */else {

    var comenHisto = document.getElementById("txtTareaHistorial").value;
    var numOp = document.getElementById("consultar").value;

    // Add a new document with a generated id.
    db.collection("comentariosHistorial").add({
      comentario: comenHisto,
      OP: numOp
    })
      .then(function (docRef) {
        ...
        Swal.fire(
          'Registro Exitoso!!!',
          'Ha enviado los comentarios de producción...',
          'success'
        )
  ...
  /* if(document.getElementById("consultar").value!="" || document.getElementById("txtTareaHistorial").value!=""){
     var comenHisto=document.getElementById("txtTareaHistorial").value;
     var numOp=document.getElementById("consultar").value;
   ...
   }else
   {
     Swal.fire(
       'Error',
       '1) Ingrese el número de OP'+'br'+'2) Presione el Boton ACTUALIZAR si ya ha hecho un registro',
       'info'
     ) 
    }*/
  // actualizarCome();
...
//---------------------------------------------------leer comentarios---------------------------------
comentariosOP = function ConsultarComen() {
  var numOp = document.getElementById("consultar").value;

  db.collection("comentariosHistorial").where("OP", "==", numOp).get()
    .then(function (querySnapshot) {
      querySnapshot.forEach(function (doc) {
        // doc.data() is never undefined for query doc snapshots
        console.log(doc.id, " => ", doc.data());
        var cmOP = `${doc.data().comentario}`;

        document.getElementById("txtTareaHistorial").value = cmOP;

        if (document.getElementById("txtTareaHistorial").value != "") {
          document.getElementById("comentariosHisto").style.display = "none";
        ...
        ActualizarComenT = function () {
          ...
          var washingtonRef = db.collection("comentariosHistorial").doc(doc.id);
          ...
          return washingtonRef.update({
            ...
            .then(function () {
              ...
              Swal.fire(
                ...
                'Ha actualizado los comentarios de producción...',
                'success'
              )
            })
            .catch(function (error) {
...
//----------------------------------------consulta por ref y modulo---------------------------------

function buscarXref() {
  var nombreRefi = document.getElementById("datoRefconsul").value;
  var nombreModu = document.getElementById("datoModuConsul").value;
...
//  cantidadElabo = 0;
  ...
  db.collection("controladministradores").where("nombreRef", "==", nombreRefi).where("usuariooperario", "==", nombreModu)
    .get()
    .then(function (querySnapshot) {
      tabl23.innerHTML = "";
      ...
      querySnapshot.forEach(function (doc) {
        ...
        console.log("las anotaciones son:" + ano);
        if (isNaN(pla)) {
          pla = 0;
        }
        if (isNaN(fi)) {
        ...
        malas = pla + fi + re + bo + manu;



        buenas = parseInt(cantidadElabo) - parseInt(malas);
        ...
        dataSet.push([OperarioFec, integrantes, inicio, fianl, numeOp, nombreReferencia, Operacion + ", " + Operacion2 + ", " + Operacion3 + ", " + Operacion4 + ", " + Operacion5 + ", " + Operacion6 + ", " + Operacion7 + ", " + Operacion8 + ", " + Operacion9, buenas, malas, cantidadElabo, "<strong>BUENAS: </strong>" + " " + buenas + "  " + "<strong>MALAS: </strong>" + " " + malas + "  " + "<i>Plana:</i>" + " " + pla + " " + " " + "<i>Fileteadora:</i>" + " " + fi + " " + " " + "<i>Recubridora</i>" + " " + re + " " + " " + "<i>B-O:</i>" + " " + bo + " " + " " + "  " + "<i>M.T:</i>" + " " + manu, (TiempoTrabajaSin / 60).toFixed(2), ((TiempoTrabajaSin / 60) * numeroOperarios).toFixed(2), pausa, ano]);
        ...
        /* tabl23.innerHTML +=`
           <tr>
       <td scope="row">${OperarioFec}</td>
       <td scope="row">${integrantes}</td>
 
       <td scope="row">${inicio}</td>
       <td scope="row">${fianl}</td>
 
 
       <td scope="row">${numeOp}</td>
     ...
     +", "+Operacion7+", "+Operacion8+", "+Operacion9}</td>
       ...
       <td>${(TiempoTrabajaSin / 60).toFixed(2)} </td>
       <td>${((TiempoTrabajaSin / 60) * numeroOperarios).toFixed(2)} </td>
       <td>${pausa} </td>
...
function consulFech2(){
  var tabl23 = document.getElementById("tabla2");

  var consultaFecha = document.getElementById("fechaConsultar2").value;
  dataSet = new Array();
  var i = 1;
  ...
  db.collection("controladministradores").where("fecha", "==", consultaFecha).orderBy("TiempoIni", "asc")
    ...
    .then(function (querySnapshot) {
      ...
      querySnapshot.forEach(function (doc) {
        ...
        dataSet.push([OperarioFec, integrantes, inicio, fianl, numeOp, nombreReferencia, Operacion + ", " + Operacion2 + ", " + Operacion3 + ", " + Operacion4 + ", " + Operacion5 + ", " + Operacion6 + ", " + Operacion7 + ", " + Operacion8 + ", " + Operacion9,color2,primer2+"-"+ultimo2,talle+"-"+lota, cantidadElabo, (TiempoTrabajaSin / 60).toFixed(2), ((TiempoTrabajaSin / 60) * numeroOperarios).toFixed(2), pausa]);
 ...
 `*/
        ...
        $(document).ready(function () {
          var tablaSuma = $('#exampleSin').DataTable({
  ...
  var limp = document.getElementById("Limpiar1");
...
function consulFech() {
  ...
  var tabl23 = document.getElementById("tablaFechas");

  var consultaFecha = document.getElementById("fechaConsultar").value;
  console.log("el formato de la fecha es"+consultaFecha);
  dataSet = new Array();
  var i = 1;
  ...
  db.collection("controladministradores").where("fecha", "==", consultaFecha).orderBy("TiempoIni", "asc")
    ...
    .then(function (querySnapshot) {
      ...
      querySnapshot.forEach(function (doc) {
        ...
        dataSet.push([OperarioFec, integrantes, inicio, fianl, numeOp, nombreReferencia, Operacion + ", " + Operacion2 + ", " + Operacion3 + ", " + Operacion4 + ", " + Operacion5 + ", " + Operacion6 + ", " + Operacion7 + ", " + Operacion8 + ", " + Operacion9,color2,primer2+"-"+ultimo2,talle+"-"+lota, buenas, malas, cantidadElabo, "<strong>BUENAS: </strong>" + " " + buenas + "  " + "<strong>MALAS: </strong>" + " " + malas + "  " + "<i>Plana:</i>" + " " + pla + " " + " " + "<i>Fileteadora:</i>" + " " + fi + " " + " " + "<i>Recubridora</i>" + " " + re + " " + " " + "<i>B-O:</i>" + " " + bo + " " + " " + "  " + "<i>M.T:</i>" + " " + manu, (TiempoTrabajaSin / 60).toFixed(2), ((TiempoTrabajaSin / 60) * numeroOperarios).toFixed(2), pausa, ano]);
...
//---------------------------------------------------------------------------------------------------------
function consul() {
  document.getElementById("comentariosHisto").style.display = "block";
  document.getElementById("comentariosHisto").style.marginLeft = "18%";
  document.getElementById("comentariosHisto").style.marginTop = "-2.7%";




  document.getElementById("txtTareaHistorial").value = "";
  tablaAdmi();
  comentariosOP();
  ...
  var consultaOP = document.getElementById("consultar").value;
  dataSet = new Array();
  var i = 1;
         ...
         .then(function(querySnapshot) {
           tabl.innerHTML="";
             ...
             querySnapshot.forEach(function(doc) {
                 // doc.data() is never undefined for query doc snapshots
                 console.log(doc.id, " => ", doc.data());
                 modu=`${doc.data().usuariooperario}`
                 tini=`${doc.data().TiempoIni}`;
                 usuarios=`${doc.data().Comen}`;
   
                  tfinal=`${doc.data().culminacion}`;
                  opera1=`${doc.data().operacionElaborada}`;
                  opera2=`${doc.data().operacionElaborada2}`;
                  opera3=`${doc.data().operacionElaborada3}`;
                  opera4=`${doc.data().operacionElaborada4}`;
                  opera5=`${doc.data().operacionElaborada5}`;
                  opera6=`${doc.data().operacionElaborada6}`;
                  opera7=`${doc.data().operacionElaborada7}`;
                  opera8=`${doc.data().operacionElaborada8}`;
                  opera9=`${doc.data().operacionElaborada9}`;
                  elabo=`${doc.data().cantidadElaborada}`;
                  ttrabajado=`${doc.data().tiempo}`;
   
                  dataSet.push([modu, usuarios,tini,tfinal,opera1+", "+opera2+", "+opera3+", "+opera4+", "+opera5+", "+opera6+", "+opera7+", "+opera8+", "+opera9 , elabo,ttrabajado ]);
   ...
   */
        ...
        //-------------------------------------hasta aqui llega la primera prueba---------------
              ...
              db.collection("FichaTecniGlobal").where("Referencia", "==", consultaref)//.where("Codigo","==",Operacion)
                ...
                .then(function (querySnapshot) {
                  querySnapshot.forEach(function (doc) {
                    ...
                    db.collection("controladministradores").where("referencia", "==", consultaOP).orderBy("culminacion", "asc")
                      .get()
                      .then(function (querySnapshot) {
                        tabl.innerHTML = "";
                        ...
                        querySnapshot.forEach(function (doc) {
                          ...
                          integrantes = `${doc.data().Comen}`;
                          lotear = `${doc.data().lote}`;
                          tallar = `${doc.data().talla}`;


                          primer = `${doc.data().Primer}`;
                          ultimo = `${doc.data().Ultimo}`;
                         ...
                         Rendi = ((rela * 100) / (tiemTrabajado - comi - tin - tin2)) / numeroOperarios;

                          TiempoTrabajaSin = tiemTrabajado - comi - tin - tin2;



                          dataSet.push([Operario, integrantes, inicio, tFinal, Operacion + ", " + Operacion2 + ", " + Operacion3 + ", " + Operacion4 + ", " + Operacion5 + ", " + Operacion6 + ", " + Operacion7 + ", " + Operacion8 + ", " + Operacion9, TiempoStdTotal,color,primer+"-"+ultimo, tallar + "-" + lotear, cantidadElabo, (TiempoTrabajaSin / 60).toFixed(2), ((TiempoTrabajaSin / 60) * numeroOperarios).toFixed(2), pausa, Math.round(Rendi)]);
                          ...
                          /*   tabl.innerHTML +=`
                                      <tr>
                                  <th scope="row">${modu}</th>
                                  <td>${usuarios}</td>
                                  <td>${tini}</td>
                                  <td>${tfinal}</td>
                                  <td>${opera1+", "+opera2+", "+opera3+", "+opera4+", "+opera5+", "+opera6+", "+opera7+", "+opera8+", "+opera9}</td>
                                  <td>Estandar</td>
                                  <td>${elabo}</td>
                                  <td>${ttrabajado}</td>
                                  <td>Pausas</td>
...
porcentajePromedio = function () {
  ...
  for (var i = 1; i < tabl3.rows.length; i++) {
    ...
    sumVal3 = sumVal3 + parseInt(tabl3.rows[i].cells[13].innerHTML);
  }

  //console.log("SUMAAAAAAAAAAAAAAAAAAAAAAAA TIEEEEEEEEEEEEEEM"+"......"+sumVal2);
  document.getElementById("porcenPromedioRendi").value = (sumVal3 / rows).toFixed(2);
...
toalTiempo = function () {
  ...
  for (var i = 1; i < tabl4.rows.length; i++) {
    ...
    sumVal4 = sumVal4 + parseFloat(tabl4.rows[i].cells[11].innerHTML);
  }

  //console.log("SUMAAAAAAAAAAAAAAAAAAAAAAAA TIEEEEEEEEEEEEEEM" + "......" + sumVal4);
  document.getElementById("TotalTiempoModulos").value = sumVal4.toFixed(2);
...
sumaTiempo = function () {
  ...
  for (var i = 1; i < tabl2.rows.length; i++) {
    ...
    sumVal2 = sumVal2 + parseFloat(tabl2.rows[i].cells[10].innerHTML);
  }

  document.getElementById("sumaTiempoTra").value = sumVal2.toFixed(2);

  //console.log("NUMERO DE FILAS USADAS"+rows);

}


sumaTiempoFech = function () {
   tabl235 = document.getElementById("regTable"), sumVal3 = 0;

  for (var i = 1; i < tabl235.rows.length; i++) {

    if (isNaN(sumVal3)) {
      sumVal3 = 0;
    }

    sumVal3 = sumVal3 + parseFloat(tabl235.rows[i].cells[14].innerHTML);
  }

  // console.log("SUMAAAAAAAAAAAAAAAAAAAAAAAA TIEEEEEEEEEEEEEEM" + "......" + sumVal3);
  document.getElementById("sumaTiempoTraFech").value = sumVal3.toFixed(2);
...
sumarCanti = function () {
  ...
  for (var i = 1; i < tabl4.rows.length; i++) {
    ...
    sumValCan = sumValCan + parseInt(tabl4.rows[i].cells[12].innerHTML);
  }


  document.getElementById("totalbtnFech").value = sumValCan;
 // console.log('la cantidad es:'+sumValCan+'el tiempo es:'+sumVal3);



}

sumarTiempoTotalFech = function () {

  var tabl4 = document.getElementById("regTable"), sumValToti = 0;
  ...
  for (var i = 1; i < tabl4.rows.length; i++) {
    ...
    sumValToti = sumValToti + parseFloat(tabl4.rows[i].cells[15].innerHTML);
  }

  // console.log("SUMAAAAAAAAAAAAAAAAAAAAAAAA TIEEEEEEEEEEEEEEM" + "......" + sumVal4);
  document.getElementById("TotalTiempoModulosFech").value = sumValToti.toFixed(2);
...
sumarMalas = function () {
  ...
  for (var i = 1; i < tabl4.rows.length; i++) {
    ...
    sumValCan = sumValCan + parseInt(tabl4.rows[i].cells[11].innerHTML);

   // console.log("Las malas son menos jajaaj:" + sumValCan);
  }


  document.getElementById("unidadesMalas").value = sumValCan;


}
/*sumarBuenas = function () {
  var tabl4 = document.getElementById("regTable"), sumValCan = 0;
  ...
  for (var i = 1; i < tabl4.rows.length; i++) {
    ...
    console.log("Las BUENAS son MAS jajaaj:"+sumValCan);
  }

  
  document.getElementById("unidadesbuenas").value = sumValCan;
...
sumarBuenas = function () {
  ...
  var cantidadTotal = document.getElementById("totalbtnFech").value;
  var cantidadMalas = document.getElementById("unidadesMalas").value;

  restaPuli = parseInt(cantidadTotal) - parseInt(cantidadMalas);
  document.getElementById("unidadesbuenas").value = restaPuli;
...
avanceProdu = function(){
  var undiadesCurrent= document.getElementById('totalbtn').value ;
  console.log('unidades actuales'+undiadesCurrent);
  var CantidaOP= document.getElementById('cantiOp').value ;
  console.log('unidades op'+CantidaOP);


  var avanceActu = (undiadesCurrent * 100)/CantidaOP ;

  console.log('avanece'+avanceActu);

  document.getElementById('avanceProdu').value =Math.round(avanceActu) ;
...
unidadesXhora=function(){
  sumValCan=0;
  var totalUds=document.getElementById('totalbtnFech').value;
  
  var totalTime=document.getElementById('sumaTiempoTraFech').value;
  console.log('unidades:'+sumValCan+'....'+'tiempo'+sumVal3);
  var udsXhora=0;
  udsXhora=((totalUds/totalTime)*60).toFixed(2);
  document.getElementById('udsXhr').value=udsXhora;
...
function sumar() {
  ...
  var tabl = document.getElementById("example"), sumVal = 0;
  ...
  for (var i = 1; i < tabl.rows.length; i++) {
    ...
    sumVal = sumVal + parseInt(tabl.rows[i].cells[9].innerHTML);
  }

  console.log("SUMAAAAAAAAAAAAAAAAAAAAAAAA" + "......" + sumVal);
  document.getElementById("totalbtn").value = sumVal;
  avanceProdu();
